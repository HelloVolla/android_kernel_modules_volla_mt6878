				...
				unsigned int sensorID)
{
	...
	if(cam_cal_config->sensor_id == IMX766_SENSOR_ID || cam_cal_config->sensor_id == OV50E40_SENSOR_ID) {
		if (read_data_region(pdata, (u8 *)&check_id, header_offset, 1) != 1) {
			debug_log("header_id read failed\n");
			return result;
		}
	}
	else if (read_data_region(pdata, (u8 *)&check_id, header_offset, 4) != 4) {
		debug_log("header_id read failed\n");
		return result;
	}
...
unsigned int gc08a3_layout_check(struct EEPROM_DRV_FD_DATA *pdata,
				unsigned int sensorID)
{
	unsigned int check_id = 0x00000000;
	unsigned int result = CAM_CAL_ERR_NO_DEVICE;
	...
	if (cam_cal_config->sensor_id == GC08A3_SENSOR_ID)
		check_id = 0x08a3;
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	table_size = 1868;	/*prize modify by zhuzhengjiang start*/
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (pCamCalData->PDAF.Data[2564] == 0x9 && pCamCalData->sensorID == IMX766_SENSOR_ID)
		pCamCalData->PDAF.Data[2564] = 0xA;
