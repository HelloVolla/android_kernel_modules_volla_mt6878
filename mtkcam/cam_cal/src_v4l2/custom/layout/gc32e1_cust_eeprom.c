...
static unsigned int do_single_lsc_gc32e1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_gc32e1(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000768, 0x00000150, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000001, 0x00000000, 0x00000000, do_module_version},
		{0x00000001, 0x00000001, 0x00000002, do_part_number},
		{0x00000001, 0x0000077B, 0x0000074C, do_single_lsc_gc32e1},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_gc32e1},
...
struct STRUCT_CAM_CAL_CONFIG_STRUCT gc32e1_cust_eeprom = {
	.name = "gc32e1_cust_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 4, (unsigned char *)&CalGain);
		if (read_data_size > 0)	{
		...
		awb_offset = 0x775;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 4, (unsigned char *)&FacGain);
		if (read_data_size > 0)	{
