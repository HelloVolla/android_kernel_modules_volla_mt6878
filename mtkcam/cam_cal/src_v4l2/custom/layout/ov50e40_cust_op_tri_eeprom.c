...
static unsigned int do_single_lsc_ov50e40(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_2a_gain_ov50e40(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);
static unsigned int do_lens_id_ov50e40(struct EEPROM_DRV_FD_DATA *pdata,
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData);

static struct STRUCT_CALIBRATION_LAYOUT_STRUCT cal_layout_table = {
	0x00000001, 0x00000050, CAM_CAL_SINGLE_EEPROM_DATA,
	{
		{0x00000000, 0x00000000, 0x00000000, do_module_version},
		{0x00000000, 0x00000000, 0x00000002, do_part_number},
		{0x00000001, 0x00000031, 0x0000074C, do_single_lsc_ov50e40},
		{0x00000001, 0x00000007, 0x0000000E, do_2a_gain_ov50e40},
		{0x00000001, 0x00000791, 0x00000A28, do_pdaf},
		{0x00000000, 0x00000FAE, 0x00000550, do_stereo_data},
		{0x00000000, 0x00000000, 0x00008000, do_dump_all},
		{0x00000000, 0x00000008, 0x00000002, do_lens_id_ov50e40}
	}
};

struct STRUCT_CAM_CAL_CONFIG_STRUCT ov50e40_cust_op_tri_eeprom = {
	.name = "ov50e40_cust_op_tri_eeprom",
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	debug_log("======================ov50e40 SingleLsc Data==================\n");
	debug_log("======================ov50e40 SingleLsc Data==================\n");
		...
		unsigned int start_addr, unsigned int block_size, unsigned int *pGetSensorCalData)
{
	...
	if (0x1 & AWBAFConfig) {
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&CalR);
		awb_offset = awb_offset+2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&CalB);
		awb_offset = awb_offset+2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&CalGr);
		awb_offset = awb_offset+2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&CalGb);
		awb_offset = awb_offset+2;
		if (read_data_size > 0)	{
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&FacR);
		awb_offset = awb_offset + 2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&FacB);
		awb_offset = awb_offset + 2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&FacGr);
		awb_offset = awb_offset + 2;
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				awb_offset, 2, (unsigned char *)&FacGb);
		awb_offset = awb_offset + 2;
		if (read_data_size > 0)	{
	...
	if (0x2 & AWBAFConfig) {
		...
		read_data_size = read_data(pdata, pCamCalData->sensorID, pCamCalData->deviceID,
				0x0781, 2, (unsigned char *)&AFMacro);
		if (read_data_size > 0)
			err = CAM_CAL_ERR_NO_ERR;
		else {
			pCamCalData->Single2A.S2aBitEn = CAM_CAL_NONE_BITEN;
