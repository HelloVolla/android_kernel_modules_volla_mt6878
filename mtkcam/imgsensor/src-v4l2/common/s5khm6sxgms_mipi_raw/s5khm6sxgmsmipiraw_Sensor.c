 ...
 *===================================================================
 *******************************************************************/
#include "s5khm6sxgmsmipiraw_Sensor.h"
#define S5KHM6SXGMS_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int s5khm6sxgms_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5khm6sxgms_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int s5khm6sxgms_sensor_init(struct subdrv_ctx *ctx);
//static int s5khm6sxgms_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
//static int s5khm6sxgms_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5khm6sxgms_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, s5khm6sxgms_set_test_pattern_data},
	//{SENSOR_FEATURE_SET_STREAMING_RESUME, s5khm6sxgms_set_streaming_resume},
	//{SENSOR_FEATURE_SET_STREAMING_SUSPEND, s5khm6sxgms_set_streaming_suspned},
};
...
//static u32 s5khm6sxgms_dcg_ratio_table_cus5[] = {8000};

//static u32 s5khm6sxgms_dcg_ratio_table_cus6[] = {8000};
...
//prize add by lipengpeng 20240531 end for PDAF
		.ae_binning_ratio = 4,
		.fine_integ_line = 0,
//drv add by lipengpeng 20240913 start 
		.delay_frame = 1, 
//drv add by lipengpeng 20240913 end 
		.csi_param = {
			.cphy_settle = 98,
		},
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = S5KHM6SXGMS_SENSOR_ID,
	...
	.ana_gain_type = 2,
	.ana_gain_step = 1,
	.ana_gain_table = s5khm6sxgms_ana_gain_table,
	.ana_gain_table_size = sizeof(s5khm6sxgms_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 3,
...
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)
    printk("s5khm6sxgms ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
  if(check_board_type()==1)
    {
	 printk(" s5khm6sxgms is volume production\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id +=1;
			printk("s5khm6sxgms i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5KHM6SXGMS_SENSOR_ID) {
...
const struct subdrv_entry s5khm6sxgms_mipi_raw_entry = {
	.name = "s5khm6sxgms_mipi_raw",
	.id = S5KHM6SXGMS_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int s5khm6sxgms_sensor_init(struct subdrv_ctx *ctx)
{
	printk("s5khm6sxgms_sensor_init start\n");
	...
	printk("s5khm6sxgms_sensor_init end \n");
...
static int open(struct subdrv_ctx *ctx)
{
    ...
    printk("s5khm6sxgms open end\n");
	
	return ERROR_NONE;
} /* open */

/*static void s5khm6sxgms_set_streaming_control(struct subdrv_ctx *ctx, bool enable)
{
...
static int s5khm6sxgms_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khm6sxgms_set_streaming_control(ctx, TRUE);
	return 0;
}
static int s5khm6sxgms_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khm6sxgms_set_streaming_control(ctx, FALSE);
	return 0;
}
	if (enable)
	{
