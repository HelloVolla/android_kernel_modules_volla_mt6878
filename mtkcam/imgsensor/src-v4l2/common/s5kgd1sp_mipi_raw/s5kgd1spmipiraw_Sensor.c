...
static int s5kgd1sp_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5kgd1sp_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void s5kgd1sp_sensor_init(struct subdrv_ctx *ctx);
static int open(struct subdrv_ctx *ctx);
//static int s5kgd1sp_set_ctrl_locker(struct subdrv_ctx *ctx, u32 cid, bool *is_lock);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5kgd1sp_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, s5kgd1sp_set_test_pattern_data},
};
...
//.mipi_pixel_rate = 1296000000,
...

	{
		...
		.framelength = 6074,
		.max_framerate = 30,
		.mipi_pixel_rate = 876000000,
		.readout_length = 0,
		.read_margin = 0,
		.imgsensor_winsize_info = { //	 { 6560, 4928,	 0,	   0, 6560, 4928, 6560, 4928,  0,   0, 6560, 4928,	  0,	0, 6560, 4928}
			.full_w = 6560,
			.full_h = 4928,
			.x0_offset = 0,
			.y0_offset = 0,
...
//drv add by lipengpeng 20240723 camera mirror filip
	...
	.ana_gain_type = 2,
	.ana_gain_step = 32,
	.ana_gain_table = s5kgd1sp_ana_gain_table,
	.ana_gain_table_size = sizeof(s5kgd1sp_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 3,
...
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)
    printk("S5KGD1SP ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
  if(check_board_type()==0)
    {
	 printk(" s5kgd1sp is volume production\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id += 0x1000;
			printk("S5KGD1SP i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5KGD1SP_SENSOR_ID) {
...
static struct subdrv_ops ops = {
	...
	//.set_ctrl_locker = s5kgd1sp_set_ctrl_locker,
};
...
const struct subdrv_entry s5kgd1sp_mipi_raw_entry = {
	.name = "s5kgd1sp_mipi_raw",
	.id = S5KGD1SP_SENSOR_ID,
	.pw_seq = pw_seq,
