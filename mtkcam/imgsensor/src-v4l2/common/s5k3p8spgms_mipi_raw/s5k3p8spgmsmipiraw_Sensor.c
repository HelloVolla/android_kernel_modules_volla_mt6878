...
static int s5k3p8spgms_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5k3p8spgms_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void s5k3p8spgms_sensor_init(struct subdrv_ctx *ctx);
static int open(struct subdrv_ctx *ctx);
//static int s5k3p8spgms_set_ctrl_locker(struct subdrv_ctx *ctx, u32 cid, bool *is_lock);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5k3p8spgms_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, s5k3p8spgms_set_test_pattern_data},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	...
	},
	{
		...
		.readout_length = 0,
		.read_margin = 0,
		.imgsensor_winsize_info = {  //	{4608, 3456, 0, 0, 4608, 3456, 4608, 3456,0000, 0000, 4608, 3456, 0, 0, 4608, 3456},	/* capture */
			.full_w = 4608,
			.full_h = 3456,
			.x0_offset = 0,
			.y0_offset = 0,
	...
	},
	{
		...
		.framelength = 3643,
		.max_framerate = 300,
		.mipi_pixel_rate = 273000000,  //mipi pixel
		.readout_length = 0,
		.read_margin = 0,
		.imgsensor_winsize_info = {  //	{4608, 3456, 0, 0, 4608, 3456, 4608, 3456,0000, 0000, 4608, 3456, 0, 0, 4608, 3456},	/* capture */
			.full_w = 4608,
		...
		//.max_framerate = 1200,		
	{
		...
		.readout_length = 0,
		.read_margin = 0,
		.imgsensor_winsize_info = { // 	{4608, 3456, 400, 664, 3840, 2160, 1280,720, 0000, 0000, 1280, 720, 0, 0, 1280, 720},	/* hight speed video */
			.full_w = 4608,
			.full_h = 3456,
			.x0_offset = 384,
			.y0_offset = 288,
	...
	},
	{
		...
		.readout_length = 0,
		.read_margin = 0,
		.imgsensor_winsize_info = { //	{4608, 3456, 400, 664, 3840, 2160, 1920, 1080,0000, 0000, 1920, 1080, 0, 0, 1920, 1080},	/* slim video */
			.full_w = 4608,
			.full_h = 3456,
			.x0_offset = 384,
			.y0_offset = 108,
...
//drv add by lipengpeng 20240723 camera mirror filip
	.sensor_output_dataformat = SENSOR_OUTPUT_FORMAT_RAW_Gr,  //   SENSOR_OUTPUT_FORMAT_RAW_B SENSOR_OUTPUT_FORMAT_RAW_Gb  SENSOR_OUTPUT_FORMAT_RAW_R SENSOR_OUTPUT_FORMAT_RAW_Gr
	.ana_gain_def = BASEGAIN * 4,
	.ana_gain_min = BASEGAIN * 1,
	.ana_gain_max = BASEGAIN * 16,
	.ana_gain_type = 2,
	.ana_gain_step = 32,
	.ana_gain_table = s5k3p8spgms_ana_gain_table,
	.ana_gain_table_size = sizeof(s5k3p8spgms_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 3,
...
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)
   printk("s5k3p8spgms ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
  if(check_board_type()==1)
    {
	 printk(" s5k3p8spgms is gms module\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id +=2;
			printk("s5k3p8spgms i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5K3P8SPGMS_SENSOR_ID) {
...
static struct subdrv_ops ops = {
	...
	//.set_ctrl_locker = s5k3p8spgms_set_ctrl_locker,
};
...
const struct subdrv_entry s5k3p8spgms_mipi_raw_entry = {
	.name = "s5k3p8spgms_mipi_raw",
	.id = S5K3P8SPGMS_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void s5k3p8spgms_sensor_init(struct subdrv_ctx *ctx)
{
	...
	printk("s5k3p8spgms_sensor_init\n");
	...
	subdrv_i2c_wr_u16(ctx, 0x602A, 0x2F38);
	subdrv_i2c_wr_u16(ctx, 0x6F12, 0x0088);
	subdrv_i2c_wr_u16(ctx, 0x6F12, 0x0D70);
	subdrv_i2c_wr_u16(ctx, 0x0202, 0x0200);
	subdrv_i2c_wr_u16(ctx, 0x0200, 0x0618);
	subdrv_i2c_wr_u16(ctx, 0x3604, 0x0002);
	subdrv_i2c_wr_u16(ctx, 0x3606, 0x0103);
	subdrv_i2c_wr_u16(ctx, 0xF496, 0x0048);
	subdrv_i2c_wr_u16(ctx, 0xF470, 0x0020);
	subdrv_i2c_wr_u16(ctx, 0xF43A, 0x0015);
	subdrv_i2c_wr_u16(ctx, 0xF484, 0x0006);
	subdrv_i2c_wr_u16(ctx, 0xF440, 0x00AF);
	subdrv_i2c_wr_u16(ctx, 0xF442, 0x44C6);
	subdrv_i2c_wr_u16(ctx, 0xF408, 0xFFF7);
	subdrv_i2c_wr_u16(ctx, 0x3664, 0x0019);
	subdrv_i2c_wr_u16(ctx, 0xF494, 0x1010);
	subdrv_i2c_wr_u16(ctx, 0x367A, 0x0100);
	subdrv_i2c_wr_u16(ctx, 0x362A, 0x0104);
	subdrv_i2c_wr_u16(ctx, 0x362E, 0x0404);
	subdrv_i2c_wr_u16(ctx, 0x32B2, 0x0008);
	subdrv_i2c_wr_u16(ctx, 0x3286, 0x0003);
	subdrv_i2c_wr_u16(ctx, 0x328A, 0x0005);
	subdrv_i2c_wr_u16(ctx, 0xF47C, 0x001F);
	subdrv_i2c_wr_u16(ctx, 0xF62E, 0x00C5);
	subdrv_i2c_wr_u16(ctx, 0xF630, 0x00CD);
	subdrv_i2c_wr_u16(ctx, 0xF632, 0x00DD);
	subdrv_i2c_wr_u16(ctx, 0xF634, 0x00E5);
	subdrv_i2c_wr_u16(ctx, 0xF636, 0x00F5);
	subdrv_i2c_wr_u16(ctx, 0xF638, 0x00FD);
	subdrv_i2c_wr_u16(ctx, 0xF63a, 0x010D);
	subdrv_i2c_wr_u16(ctx, 0xF63C, 0x0115);
	subdrv_i2c_wr_u16(ctx, 0xF63E, 0x0125);
	subdrv_i2c_wr_u16(ctx, 0xF640, 0x012D);
	subdrv_i2c_wr_u16(ctx, 0x3070, 0x0000);
	subdrv_i2c_wr_u16(ctx, 0x0B0E, 0x0000);
	subdrv_i2c_wr_u16(ctx, 0x31C0, 0x00C8);
	subdrv_i2c_wr_u16(ctx, 0x1006, 0x0004);

	printk("s5k3p8spgms_sensor_init end\n");
}
...
/*static int s5k3p8spgms_set_ctrl_locker(struct subdrv_ctx *ctx,
