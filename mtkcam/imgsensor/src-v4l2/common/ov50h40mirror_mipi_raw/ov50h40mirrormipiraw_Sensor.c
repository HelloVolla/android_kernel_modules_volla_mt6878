 ...
 *============================================================================
 ****************************************************************************/
#include "ov50h40mirrormipiraw_Sensor.h"

static void set_sensor_cali(void *arg);

static void set_group_hold(void *arg, u8 en);
//static void ov50h40mirror_set_dummy(struct subdrv_ctx *ctx);
//static int ov50h40mirror_set_max_framerate_by_scenario(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static u16 get_gain2reg(u32 gain);
static int ov50h40mirror_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void ov50h40mirror_sensor_init(struct subdrv_ctx *ctx);
static int open(struct subdrv_ctx *ctx);
//drv add by lipengpeng 20241018 start 
static int ov50h40mirror_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov50h40mirror_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, ov50h40mirror_set_test_pattern},
//drv add by lipengpeng 20241018 start 
	{SENSOR_FEATURE_SET_STREAMING_RESUME, ov50h40mirror_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, ov50h40mirror_set_streaming_suspned},
//drv add by lipengpeng 20241018 end 
...
static struct eeprom_info_struct eeprom_info[] = {
	{
		...
		.xtalk_support = false,
		.xtalk_table = data_xtalk_ov50h40mirror2q,
		.xtalk_size = ARRAY_SIZE(data_xtalk_ov50h40mirror2q),
...
//drv add by lipengpeng 20240815 start
	{
		...
		.mode_setting_table = addr_data_pair_preview_ov50h40mirror2q,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_preview_ov50h40mirror2q),
	...
	},
	{
		...
		.mode_setting_table = addr_data_pair_capture_ov50h40mirror2q,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_capture_ov50h40mirror2q),
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = addr_data_pair_capture_ov50h40mirror2q,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(addr_data_pair_capture_ov50h40mirror2q),
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 300000000,
	...
	},
	{
		...
		.mode_setting_table = addr_data_pair_video_ov50h40mirror2q,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_video_ov50h40mirror2q),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
//.max_framerate = 1200, 8192,  6144
		...
		.mode_setting_table = addr_data_pair_hs_video_ov50h40mirror2q,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_hs_video_ov50h40mirror2q),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
//.max_framerate = 600,
		...
		.mode_setting_table = addr_data_pair_slim_video_ov50h40mirror2q,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_slim_video_ov50h40mirror2q),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
...
//drv add by lipengpeng 20240914 start 
	{//custom1
		...
		.mode_setting_table = addr_data_pair_custom1_ov50h40mirror,
		.mode_setting_len = ARRAY_SIZE(addr_data_pair_custom1_ov50h40mirror),
		.seamless_switch_group = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_table = PARAM_UNDEFINED,
		.seamless_switch_mode_setting_len = PARAM_UNDEFINED,
		.hdr_mode = HDR_NONE,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 150000000,  //pclk------------------------>bu ming
		.linelength = 1380,
		.framelength = 7232, //add by lipengpeng 
		.max_framerate = 300,
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = OV50H40MIRROR_SENSOR_ID,
	...
	.ana_gain_type = 1,
	.ana_gain_step = 4,
	.ana_gain_table = ov50h40mirror_ana_gain_table,
	.ana_gain_table_size = sizeof(ov50h40mirror_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
...
//drv add by lipengpeng 20241130 start 
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)
...
#endif
//drv add by lipengpeng 20241130 end  
...
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)
    printk("ov50h40mirror ---- read_cmos_eeprom_8()=0x%x\n",read_cmos_eeprom_8(ctx,0x00000001));
    printk("ov50h40mirror ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
  if((check_board_type()==0) && (read_cmos_eeprom_8(ctx,0x00000001)== 0x69))
    {
	 printk(" ov50h40mirror is volume production\n");
   }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id +=2;
			printk("ov50h40mirror i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == OV50H40MIRROR_SENSOR_ID) {
...
const struct subdrv_entry ov50h40mirror_mipi_raw_entry = {
	.name = "ov50h40mirror_mipi_raw",
	.id = OV50H40MIRROR_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void ov50h40mirror_sensor_init(struct subdrv_ctx *ctx)
{
	...
	printk("ov50h40mirror_sensor_init\n");
...
//drv add by lipengpeng 20240828 end  
   i2c_table_write(ctx, sensor_init_addr_data_ov50h40mirror, sizeof(sensor_init_addr_data_ov50h40mirror)/sizeof(u16));
   
	printk("ov50h40mirror_sensor_init end\n");
}
//drv add by lipengpeng 20241018 start 
...
//drv add by lipengpeng 20241018 start 
static void ov50h40mirror_set_streaming_control(struct subdrv_ctx *ctx, bool enable)
{
...
static int ov50h40mirror_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	ov50h40mirror_set_streaming_control(ctx, TRUE);
	return 0;
}
static int ov50h40mirror_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	ov50h40mirror_set_streaming_control(ctx, FALSE);
	return 0;
}
//drv add by lipengpeng 20241018 end 