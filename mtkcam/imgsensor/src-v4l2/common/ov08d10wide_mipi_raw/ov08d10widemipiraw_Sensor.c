 ...
 *============================================================================
 ****************************************************************************/
#define PFX "OV08D10WIDE_camera_sensor"
...
#include "ov08d10wide_Sensor_setting.h"
#include "ov08d10wide_ana_gain_table.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = OV08D10WIDE_SENSOR_ID,

	.checksum_value = 0xd086e5a5,//test_Pattern_mode
...
static void write_shutter(struct subdrv_ctx *ctx, kal_uint32 shutter)
{
	...
	printk("0x02 = 0x%x, 0x03 = 0x%x 0x04 = 0x%x\n", read_cmos_sensor_8(ctx, 0x02), read_cmos_sensor_8(ctx, 0x03),read_cmos_sensor_8(ctx, 0x04));
...
{
	write_cmos_sensor_8(ctx, 0xfd, 0x00);
	write_cmos_sensor_8(ctx, 0x20, 0x0e);//SW Reset, need delay
	mdelay(5);
	printk("%s E\n", __func__);
		addr_data_pair_init_ov08d10wide(ctx);
	printk("%s X\n", __func__);
}
...
{
	write_cmos_sensor_8(ctx, 0xfd, 0x00);
	return ((read_cmos_sensor_8(ctx, 0x00) << 16) | (read_cmos_sensor_8(ctx, 0x01) << 8) | read_cmos_sensor_8(ctx, 0x02));
}
		      ...
		      MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	addr_data_pair_preview_ov08d10wide(ctx);
	printk("%s X\n", __func__);
	return ERROR_NONE;
}
		  ...
		  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	addr_data_pair_capture_ov08d10wide(ctx);
	printk("%s X\n", __func__);
	return ERROR_NONE;
} /* capture(ctx) */
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ctx->vblank_convert = 2504;
	addr_data_pair_video_ov08d10wide(ctx);
	printk("%s X\n", __func__);
	return ERROR_NONE;
}
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ctx->dummy_pixel = 0;
	ctx->autoflicker_en = KAL_FALSE;
	ctx->vblank_convert = 776;
	addr_data_pair_hs_video_ov08d10wide(ctx);
	printk("%s X\n", __func__);
	return ERROR_NONE;
}
			...
			MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	ctx->dummy_pixel = 0;
	ctx->autoflicker_en = KAL_FALSE;
	ctx->vblank_convert = 768;//528;
	addr_data_pair_slim_video_ov08d10wide(ctx);
	printk("%s X\n", __func__);
	return ERROR_NONE;
}
...
static kal_uint32 set_test_pattern_mode(struct subdrv_ctx *ctx, kal_bool enable)
{
	...
	if (enable) {
		...
		write_cmos_sensor_8(ctx, 0x9a, 0x10);
		write_cmos_sensor_8(ctx, 0xfd, 0x00);
	} else {
		write_cmos_sensor_8(ctx, 0xfd, 0x00);
...
static const struct subdrv_ctx defctx = {
	...
	.fast_mode_on = 1, // pri add for sat switch ae flicker
...
static struct subdrv_pw_seq_entry pw_seq[] = {
			{HW_ID_MCLK, 24, 0},
			{HW_ID_DVDD1, 1100000, 1}, // pmic_ldo for dvdd
			{HW_ID_RST, 0, 1},
...
const struct subdrv_entry ov08d10wide_mipi_raw_entry = {
	.name = "ov08d10wide_mipi_raw",
	.id = OV08D10WIDE_SENSOR_ID,
	.pw_seq = pw_seq,
