...
static int s5khp3sp_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5khp3sp_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	...
	{SENSOR_FEATURE_SET_STREAMING_RESUME, s5khp3sp_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, s5khp3sp_set_streaming_suspned},
};
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
  ...
  //  printk("s5khp3sp ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
   ...
   if(check_board_type()==0)
    {
	 printk(" s5khp3sp is volume production\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			*sensor_id = (subdrv_i2c_rd_u8(ctx, addr_h) << 8) |
			...
			//*sensor_id +=1;
			printk("s5khp3sp i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5KHP3SP_SENSOR_ID) {
...
static int s5khp3sp_sensor_init(struct subdrv_ctx *ctx)
{
	printk("s5khp3sp_sensor_init start\n");
	...
	printk("s5khp3sp_sensor_init end \n");
...
static int open(struct subdrv_ctx *ctx)
{
    ...
    printk("s5khp3sp open end\n");
	
	return ERROR_NONE;
} /* open */

static void s5khp3sp_set_streaming_control(struct subdrv_ctx *ctx, bool enable)
{
...
static int s5khp3sp_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khp3sp_set_streaming_control(ctx, TRUE);
	return 0;
}
static int s5khp3sp_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khp3sp_set_streaming_control(ctx, FALSE);
	return 0;
}
