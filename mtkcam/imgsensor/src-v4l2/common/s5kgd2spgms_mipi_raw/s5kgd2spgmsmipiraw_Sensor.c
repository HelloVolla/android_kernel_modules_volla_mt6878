 ...
 *============================================================================
 ****************************************************************************/
#include "s5kgd2spgmsmipiraw_Sensor.h"

#define USING_DPHY_N_LANE 4

static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int s5kgd2spgms_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5kgd2spgms_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static void s5kgd2spgms_sensor_init(struct subdrv_ctx *ctx);
static int open(struct subdrv_ctx *ctx);
//static int s5kgd2spgms_set_ctrl_locker(struct subdrv_ctx *ctx, u32 cid, bool *is_lock);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5kgd2spgms_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, s5kgd2spgms_set_test_pattern_data},
};
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = S5KGD2SPGMS_SENSOR_ID,
...
//drv add by lipengpeng 20240723 camera mirror filip
	...
	.ana_gain_type = 2,
	.ana_gain_step = 32,
	.ana_gain_table = s5kgd2spgms_ana_gain_table,
	.ana_gain_table_size = sizeof(s5kgd2spgms_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 3,
    ...
    .init_setting_table = s5kgd2_init_setting,
	.init_setting_len = ARRAY_SIZE(s5kgd2_init_setting),
	.mode = mode_struct,
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
    ...
    printk("S5KGD2SPGMS ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
  if(check_board_type()==1)
    {
	 printk(" s5kgd2spgms is volume production\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id +=1;
			printk("S5KGD2SPGMS i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5KGD2SPGMS_SENSOR_ID) {
...
static struct subdrv_ops ops = {
	...
	//.set_ctrl_locker = s5kgd2spgms_set_ctrl_locker,
};
...
const struct subdrv_entry s5kgd2spgms_mipi_raw_entry = {
	.name = "s5kgd2spgms_mipi_raw",
	.id = S5KGD2SPGMS_SENSOR_ID,
	.pw_seq = pw_seq,
...
static void s5kgd2spgms_sensor_init(struct subdrv_ctx *ctx)
{
	...
	printk("s5kgd2spgms_sensor_init\n");
	subdrv_i2c_wr_u16(ctx, 0x6028, 0x4000);
	subdrv_i2c_wr_u16(ctx, 0x0000, 0x0007);
	subdrv_i2c_wr_u16(ctx, 0x0000, 0x0842);
	subdrv_i2c_wr_u16(ctx, 0x001E, 0x0300);
	//Init	
	subdrv_i2c_wr_u16(ctx, 0x6028, 0x4000);
   ...
   i2c_table_write(ctx, s5kgd2_init_setting, sizeof(s5kgd2_init_setting)/sizeof(u16));

	printk("s5kgd2spgms_sensor_init end\n");
}
