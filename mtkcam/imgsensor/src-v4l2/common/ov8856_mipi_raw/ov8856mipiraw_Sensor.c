...
static int ov8856_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int ov8856_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static void ov8856_set_streaming_control(struct subdrv_ctx *ctx, bool enable);
static int ov8856_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);

/* STRUCT */

static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_STREAMING_RESUME, ov8856_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, ov8856_set_streaming_suspned},
	{SENSOR_FEATURE_SET_TEST_PATTERN,ov8856_set_test_pattern},
};
...
static struct subdrv_mode_struct mode_struct[] = {
	// frame_desc_prev
	{
		...
		.mode_setting_table = ov8856_preview_setting,
		.mode_setting_len = ARRAY_SIZE(ov8856_preview_setting),
...
static struct subdrv_static_ctx static_ctx = {
	...
	.reg_addr_sensor_id = {0x300B, 0x300C}, //by lipengpeng ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
	.i2c_addr_table = {0x20,0x6c,0x42,0xFF},//by lipengpeng
	.i2c_burst_write_support = TRUE,
	.i2c_transfer_data_type = I2C_DT_ADDR_16_DATA_8,  //by lipengpeng add16 data8
	...
	.mipi_lane_num = SENSOR_MIPI_4_LANE, //4 line
	.ob_pedestal = 0x40,
	...
	.ana_gain_type = 3,
	.ana_gain_step = 1,
	.ana_gain_table = ov8856_ana_gain_table,
	.ana_gain_table_size = sizeof(ov8856_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 8,
	...
	.reg_addr_stream = 0x0100, //by lipengpeng write_cmos_sensor(0x0100, 0x00);
	.reg_addr_mirror_flip = PARAM_UNDEFINED,
	.reg_addr_exposure = {{0x3500,0x3501, 0x3502},},//by lipengpeng write_cmos_sensor(0x3502, (shutter << 4) & 0xFF);write_cmos_sensor(0x3501, (shutter >> 4) & 0xFF);write_cmos_sensor(0x3500, (shutter >> 12) & 0x0F);

	.long_exposure_support = FALSE,
	.reg_addr_exposure_lshift = PARAM_UNDEFINED,
	.reg_addr_ana_gain = {{0x3508, 0x3509},},  //by lipengpeng 	write_cmos_sensor(0x3508, (reg_gain>>8));write_cmos_sensor(0x3509, (reg_gain&0xFF));
	.reg_addr_frame_length = {0x380E, 0x380F},  //by lipengpegn 		write_cmos_sensor(0x380e, imgsensor.frame_length >> 8);write_cmos_sensor(0x380f, imgsensor.frame_length & 0xFF);
	...
	.init_setting_table = ov8856_init_setting,
	.init_setting_len = ARRAY_SIZE(ov8856_init_setting),
	.mode = mode_struct,
...
const struct subdrv_entry ov8856_mipi_raw_entry = {
...
static u16 get_gain2reg(u32 gain)
{
	...
	//iReg = gain*2; // by lipengpeng 20250408
	return iReg;
}
static int ov8856_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	u32 mode = *((u32 *)para);
	printk("ov8856 mode(%u->%u)\n", ctx->test_pattern, mode);
	/* 1:Solid Color 2:Color Bar 5:Black */
//drv add by lipengpeneg 20250408 start 
	if(mode) {
		printk("ov8856 switch (mode)\n");
		subdrv_i2c_wr_u8(ctx, 0x5000, 0x57);
		subdrv_i2c_wr_u8(ctx, 0x5001, 0x02);
		subdrv_i2c_wr_u8(ctx, 0x5e00, 0x80);
	}
	else {
		subdrv_i2c_wr_u8(ctx, 0x5000, 0x77);
		subdrv_i2c_wr_u8(ctx, 0x5001, 0x0a);
		subdrv_i2c_wr_u8(ctx, 0x5e00, 0x00);
	}
	//if (enable) {
	...
	//}
//drv add by lipengpeneg 20250408 end 
	ctx->test_pattern = mode;
	return ERROR_NONE;
}
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id)
...
static int ov8856_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	ov8856_set_streaming_control(ctx, TRUE);
	return 0;
}
static int ov8856_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	ov8856_set_streaming_control(ctx, FALSE);
	return 0;
}
