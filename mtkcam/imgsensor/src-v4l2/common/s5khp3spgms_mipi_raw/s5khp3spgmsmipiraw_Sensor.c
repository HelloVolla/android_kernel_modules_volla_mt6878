 ...
 *===================================================================
 *******************************************************************/
#include "s5khp3spgmsmipiraw_Sensor.h"
#define S5KHP3SPGMS_LOG_INF(format, args...) pr_info(LOG_TAG "[%s] " format, __func__, ##args)
static void set_group_hold(void *arg, u8 en);
static u16 get_gain2reg(u32 gain);
static int s5khp3spgms_set_test_pattern(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5khp3spgms_set_test_pattern_data(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int init_ctx(struct subdrv_ctx *ctx,	struct i2c_client *i2c_client, u8 i2c_write_id);
static int s5khp3spgms_sensor_init(struct subdrv_ctx *ctx);
static int s5khp3spgms_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len);
static int s5khp3spgms_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len);
...
static struct subdrv_feature_control feature_control_list[] = {
	{SENSOR_FEATURE_SET_TEST_PATTERN, s5khp3spgms_set_test_pattern},
	{SENSOR_FEATURE_SET_TEST_PATTERN_DATA, s5khp3spgms_set_test_pattern_data},
	{SENSOR_FEATURE_SET_STREAMING_RESUME, s5khp3spgms_set_streaming_resume},
	{SENSOR_FEATURE_SET_STREAMING_SUSPEND, s5khp3spgms_set_streaming_suspned},
};
...
static u32 s5khp3spgms_dcg_ratio_table_cus5[] = {8000};

static u32 s5khp3spgms_dcg_ratio_table_cus6[] = {8000};
...
//drv add by lipengpeng 20240606 end 
	...
	{//custom2 LN2
		...
		.seamless_switch_mode_setting_table = s5khp3spgms_seamless_custom2,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(s5khp3spgms_seamless_custom2),
	...
	{//custom3 2X
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = s5khp3spgms_seamless_custom3,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(s5khp3spgms_seamless_custom3),
		.hdr_mode = PARAM_UNDEFINED,
		.raw_cnt = 1,
		.exp_cnt = 1,
		.pclk = 2000000000,
	...
	{//custom4 4X
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = s5khp3spgms_seamless_custom4,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(s5khp3spgms_seamless_custom4),
		.raw_cnt = 1,
		.exp_cnt = 1,
		.hdr_mode = HDR_NONE,
		.pclk = 2000000000,
	...
	{//custom5 capture iDCG
		...
		.seamless_switch_group = 1,
		.seamless_switch_mode_setting_table = s5khp3spgms_seamless_custom5,
		.seamless_switch_mode_setting_len = ARRAY_SIZE(s5khp3spgms_seamless_custom5),
		.raw_cnt = 1,
		.exp_cnt = 1,
		...
		.dcg_info = {
			...
			.dcg_gain_table = s5khp3spgms_dcg_ratio_table_cus5,
			.dcg_gain_table_size = sizeof(s5khp3spgms_dcg_ratio_table_cus5),
		},
	},
	{//custom6 video iDCG
		...
		.dcg_info = {
			...
			.dcg_gain_ratio_max = 8000,
			.dcg_gain_ratio_step = 0,
			.dcg_gain_table = s5khp3spgms_dcg_ratio_table_cus6,
			.dcg_gain_table_size = sizeof(s5khp3spgms_dcg_ratio_table_cus6),
		},
	},
		{//custom7 200m
...
static struct subdrv_static_ctx static_ctx = {
	.sensor_id = S5KHP3SPGMS_SENSOR_ID,
	...
	.ana_gain_type = 2,
	.ana_gain_step = 1,
	.ana_gain_table = s5khp3spgms_ana_gain_table,
	.ana_gain_table_size = sizeof(s5khp3spgms_ana_gain_table),
	.min_gain_iso = 100,
	.exposure_def = 0x3D0,
	.exposure_min = 3,
...
static int get_imgsensor_id(struct subdrv_ctx *ctx, u32 *sensor_id)
{
  ...
  //  printk("s5khp3sp ---- gms_detection_getadc_v()=%d\n",gms_detection_getadc_v());
#if IS_ENABLED(CONFIG_DRV_ADC_GMS_DETECT)  
if(check_board_type()==1)
    {
	 printk(" s5khp3spgms is gms module\n");
    }else{
	...
	while (ctx->s_ctx.i2c_addr_table[i] != 0xFF) {
		...
		do {
			...
			*sensor_id +=1;
			printk("s5khp3spgms i2c_write_id(0x%x) sensor_id(0x%x/0x%x)\n",ctx->i2c_write_id, *sensor_id, ctx->s_ctx.sensor_id);
			if (*sensor_id == S5KHP3SPGMS_SENSOR_ID) {
...
static struct subdrv_ops ops = {
	.get_id = get_imgsensor_id, //drv add by lipengpeng 20240601 
	.init_ctx = init_ctx,
	.open = open,
...
const struct subdrv_entry s5khp3spgms_mipi_raw_entry = {
	.name = "s5khp3spgms_mipi_raw",
	.id = S5KHP3SPGMS_SENSOR_ID,
	.pw_seq = pw_seq,
...
static int s5khp3spgms_sensor_init(struct subdrv_ctx *ctx)
{
	printk("s5khp3spgms_sensor_init start\n");
	...
	printk("s5khp3spgms_sensor_init end \n");
...
static int open(struct subdrv_ctx *ctx)
{
    ...
    printk("s5khp3spgms open end\n");
	
	return ERROR_NONE;
} /* open */

static void s5khp3spgms_set_streaming_control(struct subdrv_ctx *ctx, bool enable)
{
...
static int s5khp3spgms_set_streaming_resume(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khp3spgms_set_streaming_control(ctx, TRUE);
	return 0;
}
static int s5khp3spgms_set_streaming_suspned(struct subdrv_ctx *ctx, u8 *para, u32 *len)
{
	s5khp3spgms_set_streaming_control(ctx, FALSE);
	return 0;
}
