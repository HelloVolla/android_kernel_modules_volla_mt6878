...
/*pri add by zhuzhengjiang start*/
#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
	static int num_camera=0;
	MSDK_SENSOR_RESOLUTION_INFO_STRUCT sensor_resolution;
#endif
/*pri add by zhuzhengjiang end*/
	...
	for (i = 0; i < subdrvs_cnt; i++) {
		...
		/*pri add by zhuzhengjiang start*/
		#if IS_ENABLED(CONFIG_PRIZE_HARDWARE_INFO)
			ret = subdrv_call(ctx, get_resolution, &sensor_resolution);
			if(strcmp(ctx->subdrv->name,"ov64b40_mipi_raw")==0) {
				strcpy(current_camera_info[0].chip,ctx->subdrv->name);
				sprintf(current_camera_info[0].id,"0x%04x",sensor_id);
				sprintf(current_camera_info[0].more,"%d*%d",9248,6944);
				strcpy(current_camera_info[0].vendor,"Huizhou Xinhengwei");
			} else if(strcmp(ctx->subdrv->name,"gc32e1sub_mipi_raw")==0) {
				strcpy(current_camera_info[1].chip,ctx->subdrv->name);
				sprintf(current_camera_info[1].id,"0x%04x",sensor_id);
				sprintf(current_camera_info[1].more,"%d*%d",6528,4896);
				strcpy(current_camera_info[1].vendor,"Chongqing TS-Precision");
			} else if(strcmp(ctx->subdrv->name,"gc08a3_mipi_raw")==0) {
				strcpy(current_camera_info[2].chip,ctx->subdrv->name);
				sprintf(current_camera_info[2].id,"0x%04x",sensor_id);
				sprintf(current_camera_info[2].more,"%d*%d",sensor_resolution.SensorWidth[1],sensor_resolution.SensorHeight[1]);
				strcpy(current_camera_info[2].vendor,"Luzhou Hedayuan");
			} else {
				strcpy(current_camera_info[num_camera].chip,ctx->subdrv->name);
				sprintf(current_camera_info[num_camera].id,"0x%04x",sensor_id);
				sprintf(current_camera_info[num_camera].more,"%d*%d",1600,1200);
				strcpy(current_camera_info[num_camera].vendor,"Luzhou C&T");
			}
			num_camera++;
		#endif
		/*pri add by zhuzhengjiang end*/
