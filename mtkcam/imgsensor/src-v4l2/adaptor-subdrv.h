...
enum {
	...
	I2C_DT_ADDR_8_DATA_8, // pri add by zhuzhengjiang for addr 8 data 8
	I2C_DT_MAXCNT,
};
...
struct subdrv_ctx {
	...
	u16 vblank_convert; // pri add by zhuzhengjiang for vblank
...
#define subdrv_i2c_rd_addr8_u8(subctx, reg) \
({ \
	u8 __val = 0xff; \
	adaptor_i2c_rd_add8_u8(subctx->i2c_client, \
		subctx->i2c_write_id >> 1, reg, &__val); \
	__val; \
})
#define subdrv_i2c_wr_addr8_u8(subctx, reg, val) \
	adaptor_i2c_wr_addr8_u8(subctx->i2c_client, \
		subctx->i2c_write_id >> 1, reg, val)
#define subdrv_i2c_wr_regs_addr8_u8(subctx, list, len) \
	adaptor_i2c_wr_regs_addr8_u8(subctx->i2c_client, \
