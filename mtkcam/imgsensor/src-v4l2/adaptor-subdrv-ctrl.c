...
void i2c_table_write(struct subdrv_ctx *ctx, u16 *list, u32 len)
{
	...
	case I2C_DT_ADDR_8_DATA_8:
		subdrv_i2c_wr_regs_addr8_u8(ctx, list, len);
		break;
	/*pri add by zhuzhengjiang start*/
...
void set_gain(struct subdrv_ctx *ctx, u32 gain)
{
	...
	if (gph && !ctx->ae_ctrl_gph_en)
		ctx->s_ctx.s_gph((void *)ctx, 1);
	/* write gain */
	/*prize add by zhuzhengjiang for ov50e40 ae line start*/
	DRV_LOG(ctx, "ctx->s_ctx.sensor_id:0x%x rg_gain:0x%x ctx->current_scenario_id:%d \n", ctx->s_ctx.sensor_id,rg_gain,ctx->current_scenario_id);
	if(ctx->current_scenario_id != 16 && ctx->s_ctx.sensor_id == OV50E40_SENSOR_ID) {
		if(rg_gain >= 0x400)
			set_i2c_buffer(ctx,	0x3506,0xfb);
		else
			set_i2c_buffer(ctx,	0x3506,0xf9);
	}
	/*prize add by zhuzhengjiang for ov50e40 ae line end*/
