...
/* prize add by chenwenhui for cam start */
extern int wl28681_set_ldo(int sensoridx,int enable);
/* prize add by chenwenhui for cam end */
...
int do_hw_power_on(struct adaptor_ctx *ctx)
{
	...
	for (i = 0; i < ctx->subdrv->pw_seq_cnt; i++) {
		...
		adaptor_logd(ctx,"ctx->subdrv->name:%s ent->id:%d  ent->val:%d\n",ctx->subdrv->name, ent->id, ent->val);
		if (strcmp(ctx->subdrv->name,"ov64b40_mipi_raw") == 0 && ent->id == HW_ID_DOVDD) {
			wl28681_set_ldo(0,1);
		} else if (strcmp(ctx->subdrv->name,"gc32e1sub_mipi_raw") == 0 && ent->id == HW_ID_DOVDD) {
		    wl28681_set_ldo(1,1);
		} else if (strcmp(ctx->subdrv->name,"gc08a3_mipi_raw") == 0 && ent->id == HW_ID_DOVDD) {
		    wl28681_set_ldo(2,1);
		} else if (strcmp(ctx->subdrv->name,"gc02m1macro_mipi_raw") == 0 && ent->id == HW_ID_DOVDD) {
		    wl28681_set_ldo(3,1);
		}
