...
#define MAX_MSG_NUM_ADDR8_U8 (MAX_BUF_SIZE / 2)/*pri add by zhuzhengjiang start*/
...
struct cache_wr_regs_addr8_u8 {
	u8 buf[MAX_BUF_SIZE];
	struct i2c_msg msg[MAX_MSG_NUM_ADDR8_U8];
};
int adaptor_i2c_rd_add8_u8(struct i2c_client *i2c_client,
...
int adaptor_i2c_wr_addr8_u8(struct i2c_client *i2c_client,
		u16 addr, u8 reg, u8 val)
{
	int ret;
	u8 buf[2];
...
int adaptor_i2c_wr_regs_addr8_u8(struct i2c_client *i2c_client,
		u16 addr, u16 *list, u32 len)
{
	struct cache_wr_regs_addr8_u8 *pmem;
	struct i2c_msg *pmsg;
	u8 *pbuf;
	u16 *plist;
