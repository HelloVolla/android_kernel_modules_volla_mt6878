...
/*Drv: add for bug KMXFS-1169 start*/
	//*temp = temp_data.u4Temperature;
	//sensor->updated_period = jiffies +
	//	wifi_var->u4MinTempQueryTime * HZ / 1000;
	//sensor->last_query_temp = *temp;
	if (temp_data.u4Temperature > RESET_TEMP_THRESHOLD) {
		DBGLOG(REQ, INFO, "temp(%u) is too high\n",
			temp_data.u4Temperature);
		*temp = THERMAL_TEMP_INVALID;
		GL_USER_DEFINE_RESET_TRIGGER(ad,
			RST_HIGH_TEMPERATURE, RST_FLAG_WHOLE_RESET);
	} else {
		*temp = temp_data.u4Temperature;
		sensor->updated_period = jiffies + wifi_var->u4MinTempQueryTime * HZ / 1000;
		sensor->last_query_temp = *temp;
	}
	
	printk("temp is %d, i4MaxTempLimit is %d\n",*temp,wifi_var->i4MaxTempLimit);

exit:
	//if (*temp >= (wifi_var->i4MaxTempLimit)){
		DBGLOG(REQ, TRACE, "idx[%d] type[%d] name[%s], temp[%d][%d]\n",
			...
			*temp);
	//}
/*Drv: add for bug KMXFS-1169 end*/	
	wfsys_unlock();
	return 0;
}
